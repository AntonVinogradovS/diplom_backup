...
def kdedensity(i1, i2, j1, j2, iter):
    ...
    #             else:
    #                 D[t, x, y] = cellOver[t,x,y] - cellOver[t-1,x,y]
    #print(D)
    if iter == 1:
        i1, i2, j1, j2 = 0,10,10,20 #вне
        nameTitle = "KDE вне клетки"
        nameFile = "./diplom/myImage/KDEoutSide3.pdf"
    elif iter == 2:
        i1, i2, j1, j2 = 21,32,37,45 # право
        nameTitle = "KDE Правая клетка(эксперимент2)"
        nameFile = "./diplom/myImage/KDE Правая клетка(эксперимент2).png"
    elif iter == 3:
        i1, i2, j1, j2 = 38,48,0,10 # низ
        
        nameTitle = "KDE для нижней клетки"
        nameFile = "./diplom/myImage/KDEdown3.pdf"
    elif iter == 4:
        i1, i2, j1, j2 = 14,25,14,28 # верх
        nameTitle = "KDE Верхняя клетка(эксперимент2)"
        nameFile = "./diplom/myImage/KDE Верхняя клетка(эксперимент2).png"
    else:
        i1, i2, j1, j2 = 27,36,16,28 # центр
        nameTitle = "KDE Центральная клетка(эксперимент2)"
        nameFile = "./diplom/myImage/KDE Центральная клетка(эксперимент2).png"
    ...
    pdf = PdfPages(nameFile)
    im = ax.imshow(zNew, origin='lower', cmap=plt.cm.jet, interpolation='nearest', extent=extent, aspect='auto')
